...
for (meteostation in stazione){
  ...
  for (sensore_code in sensoren) {
    ...
    if (NROW(pippo)!=0) {
      ...
      if (sensore_code == "N" ) {pippo <- setNames(data.frame(matrix(ncol = 2, nrow = 0)), c("DATE", "N"))  ; pluto <- merge(pluto,pippo,by="DATE")}
      if (sensore_code == "SD") {pippo <- setNames(data.frame(matrix(ncol = 2, nrow = 0)), c("DATE", "SD")) ; pluto <- merge(pluto,pippo,by="DATE")}
    }
